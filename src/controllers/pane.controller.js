...
const POPULATE_FIELDS = ['order', 'withdrawal', 'remakeOf'];
...
exports.getAll = async (req, res, next) => {
  try {
    const { data, pagination } = await paginate(GlassPane, {
...
exports.getById = async (req, res, next) => {
  try {
    const pane = await GlassPane.findById(req.params.id).populate(POPULATE_FIELDS);
...
exports.create = async (req, res, next) => {
  try {
    const { order, withdrawal, remakeOf } = req.validated.body;
    await verifyReferences([
      { model: Order, id: order, label: 'Order' },
      { model: Withdrawal, id: withdrawal, label: 'Withdrawal' },
      { model: GlassPane, id: remakeOf, label: 'GlassPane (remakeOf)' },
    ]);

    const paneNumber = await Counter.getNext('pane', 'PNE');
    const qrCode = `STDPLUS:${paneNumber}`;
    const pane = await GlassPane.create({ ...req.validated.body, paneNumber, qrCode });
...
exports.update = async (req, res, next) => {
  try {
    ...
    const pane = await GlassPane.findByIdAndUpdate(req.params.id, req.validated.body, {
      new: true,
      runValidators: true,
    }).populate(POPULATE_FIELDS);
    if (!pane) return fail(res, 'Pane not found', 404);
...
exports.deleteOne = async (req, res, next) => {
  try {
    ...
    const pane = await GlassPane.findByIdAndDelete(req.params.id);
    if (!pane) return fail(res, 'Pane not found', 404);
...
exports.deleteMany = async (req, res, next) => {
  try {
    ...
    const result = await GlassPane.deleteMany({ _id: { $in: ids } });
